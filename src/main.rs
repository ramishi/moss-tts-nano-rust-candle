...
/// Resolve a model file: download from HF (or use cache), fall back to local path if exists.
fn resolve_model_path(filename: &str, local_default: &Path) -> Result<PathBuf> {
    ...
    if let Ok(api) = Api::new() {
        ...
        match repo.get(filename) {
            ...
            Err(e) => {
                println!(
                    "[LOCAL] HF download failed for '{}': {}, trying local path",
                    filename, e
                );
            }
        }
    } else {
        println!("[LOCAL] HF API init failed, trying local path");
    }

    // Second try: local path as-is
    if local_default.exists() {
        return Ok(local_default.to_path_buf());
    }

    // Third try: models_backup_YYYYMMDDHHMMSS/ directory
    ...
    anyhow::bail!(
        "Model file '{}' not found.\n\
         Tried: HF cache (ramishi/moss-tts-nano-candle) and local path.\n\
         Ensure internet connection or provide explicit local paths.",
        filename
    )
}
...
struct Args {
    ...
    /// Skip HF download, use only local paths (for offline use).
    #[arg(long, default_value_t = false)]
    local_only: bool,
}
...
fn main() -> Result<()> {
    ...
    let tts = if args.local_only {
        println!("[LOCAL-ONLY] Skipping HF download, using explicit paths only.");
        ...
        // Resolve model paths: HF cache → local → backup
